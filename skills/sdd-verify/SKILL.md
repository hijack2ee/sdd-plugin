---
name: sdd-verify
description: Run lint, type-check, and tests as a gate before shipping. Reads verify commands from .specs/config.yaml, executes in order, records result in manifest.ci. Pass is required for sdd-ship. Trigger phrases - "/sdd-verify", "run verify", "lint and test", "check before PR".
---
